...
def speak(text):
    ...
    engine.say(text)
    engine.runAndWait()
def speak_translated(text):
    global preferred_lang
    translated = translator.translate(text, dest=preferred_lang).text
    print(f"In {preferred_lang}: {translated}")
    speak(translated)
