...
def play_media(category, name):
    category = category.lower()
    name = name.lower()

    if category in media.media:
        link = media.media[category].get(name)
        if link:
            speak(f"Playing {name} from {category}")
            webbrowser.open(link)
        else:
            speak(f"'{name}' not found in {category}.")
    else:
        speak(f"The category '{category}' doesn't exist.")
